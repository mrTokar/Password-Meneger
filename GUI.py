...
class Note:
    ...
    def open_window(self, master=None):
        ...
        self.password_window.geometry(
            f"500x200+{self.password_window.winfo_screenwidth() // 2 - 250}+{self.password_window.winfo_screenheight() // 2 - 100}")
        ...
        if (master is not None) or self.disable_del:  # если объект новый, то удалить его нельзя
            del_btn.disable()
            self.disable_del = False
